...
export default function ResourcesPage() {
  return (
    <main>
      ...
      <section className="page-content"><div className="section-inner grid-3">{blogPosts.map((post) => <Link className="card" href={`/resources/${post.slug}`} key={post.slug}><h2>{post.title}</h2><p>{post.excerpt}</p></Link>)}</div></section>
    </main>
  );
}
